...
Task frames may also carry `controller_overrides`. The field stays generic in shared config and serialization code, while each robot wrapper translates the supported keys into concrete controller commands. UR also exposes a first-class `stiffness_mode`, so task-space vs joint-space remains geometric while primitives can still choose compliant (`forceMode` / `directTorque`) or stiff (`servoL` / `servoJ`) UR execution.
